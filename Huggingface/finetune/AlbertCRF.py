...
class AlbertCrf(AlbertPreTrainedModel):
    ...
    def __init__(self, config:AlbertConfig):
        ...
        # 这里使用albert
        self.bert = AlbertModel(config, add_pooling_layer=False)


        self.birnn = nn.GRU(config.hidden_size,  # Note emb = word + char
                            Hyper.birnn_hidden_state,
                            num_layers=2,  # Note
                            bidirectional=True,
                            batch_first=True,
        ...
        out_dim = Hyper.birnn_hidden_state * 2
        ...
        if Hyper.using_attention:
            ...
            attentionB_config.hidden_size = Hyper.birnn_hidden_state * 2
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # Todo 句子分类任务, 判别句子是否出现了实体

        logits = emissions
        # Todo 怎么设置多个损失函数的权重
        return TokenClassifierOutput(
            loss=loss + 100 * loss_tokenCLS + 10 * loss_simpleNER,
